...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      ...
      <div className="fixed z-[110] right-3 sm:right-5 top-1/2 -translate-y-1/2 flex flex-col gap-3 items-end dir-ltr select-none">
        ...
        <div className="relative group/btn flex items-center">
          ...
          <button
            ...
            className="w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-black hover:bg-[#006063] text-white border border-white/20 shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-110 cursor-pointer active:scale-95"
      ...
      {/* Main Header Wrapper */}
      <header className={`sticky top-0 z-[100] w-full bg-[#e6e6e6] border-b border-gray-300 shadow-sm transition-transform duration-300 ease-in-out ${
        headerVisible ? 'translate-y-0' : '-translate-y-full'
      }`}>
        
        {/* Top Utility Nav Bar (Exact Biesse top bar) */}
        <div className="bg-[#e6e6e6] px-4 sm:px-10 pt-3 pb-2 hidden lg:block border-b border-gray-200">
          <div className="max-w-[1440px] mx-auto flex justify-between items-center text-xs font-medium text-slate-800">
            
            {/* Left Tagline */}
            <div className="flex items-center gap-2 text-slate-500">
              <span className="w-2 h-2 rounded-full bg-[#006063]" />
              <span>{isFa ? 'سامانه بین‌المللی تولید دستگاه‌ها و کدهای وب بیئس' : 'Biesse Group • Industrial Processing & Digital Architecture'}</span>
            </div>

            {/* Right Top Links */}
            <div className="flex items-center gap-5">
              <a href="#about" className="hover:underline hover:text-[#006063] flex items-center gap-1">
                <span>{isFa ? 'درباره شرکت' : 'Company'}</span>
                <ChevronDown className="w-3 h-3 text-slate-400" />
              </a>
              <a href="#sustainability" className="hover:underline hover:text-[#006063] flex items-center gap-1">
                <span>{isFa ? 'پایداری و نوآوری' : 'Sustainability'}</span>
                <ChevronDown className="w-3 h-3 text-slate-400" />
              </a>
              <a href="#locations" className="hover:underline hover:text-[#006063] flex items-center gap-1">
                <span>{isFa ? 'شعب و نمایندگی‌ها' : 'Governance & Investor'}</span>
                <ChevronDown className="w-3 h-3 text-slate-400" />
              </a>
              <Link to={`/${lang}/blog`} className="hover:underline hover:text-[#006063]">
                <span>{isFa ? 'اخبار و مقالات' : 'News'}</span>
              </Link>
              <a href="#contact" className="hover:underline hover:text-[#006063]">
                <span>{isFa ? 'تماس با ما' : 'Contact us'}</span>
              </a>

              {/* Language Selector Button */}
              <div className="inline-flex border border-black py-0.5 px-3 rounded-full uppercase text-xs font-bold items-center gap-1 cursor-pointer select-none bg-white/80 hover:bg-white text-black transition">
                <Globe className="w-3.5 h-3.5 text-[#006063]" />
                <button onClick={() => onLanguageChange('en')} className={`hover:text-[#006063] ${!isFa ? 'text-[#006063]' : ''}`}>
                  ww-en
                </button>
                <span>/</span>
                <button onClick={() => onLanguageChange('fa')} className={`hover:text-[#006063] ${isFa ? 'text-[#006063]' : ''}`}>
                  fa-IR
                </button>
              </div>
            </div>
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-10 py-3.5 flex items-center justify-between gap-6">
          ...
          {/* Biesse Brand SVG Logo */}
          <Link to={`/${lang}`} className="flex items-center gap-3 shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 219 65" className="h-10 sm:h-12 text-[#006063]">
          ...
          <nav className="hidden lg:flex items-center gap-8 text-black text-lg xl:text-xl font-bold">
            ...
            <div className="btn--watermark shadow-sm relative group bg-white px-4 py-1.5 rounded-md border border-gray-300">
              <a href="#contact" className="hover:text-[#006063] flex items-center gap-1 text-[#006063]">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onOpenExporter}
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-gray-300 text-[#006063] hover:bg-[#006063] hover:text-white transition shadow-sm text-xs font-bold"
            ...
            <a
              href="#locations"
              className="w-9 h-9 rounded-full bg-white border border-gray-300 flex items-center justify-center text-slate-800 hover:text-[#006063] hover:border-[#006063] transition shadow-sm"
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg bg-white border border-gray-300 text-slate-800 hover:text-[#006063]"
            >
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-t border-gray-200 px-6 py-6 space-y-4 shadow-xl">
            <nav className="flex flex-col space-y-3 text-slate-900 font-bold text-base">
              <a href="#materials" onClick={() => setMobileMenuOpen(false)} className="hover:text-[#006063]">
                {isFa ? 'خطوط تولید (Lines)' : 'Lines'}
              </a>
              <a href="#services" onClick={() => setMobileMenuOpen(false)} className="hover:text-[#006063]">
                {isFa ? 'دستگاه‌ها و کدهای PHP (Machines)' : 'Machines'}
              </a>
              <a href="#portfolio" onClick={() => setMobileMenuOpen(false)} className="hover:text-[#006063]">
                {isFa ? 'کامپوننت‌ها و دیتابیس (Components)' : 'Components'}
              </a>
              <a href="#contact" onClick={() => setMobileMenuOpen(false)} className="hover:text-[#006063]">
                {isFa ? 'پشتیبانی مشتریان (Customer Care)' : 'Customer Care'}
              </a>
              <a href="#blog" onClick={() => setMobileMenuOpen(false)} className="hover:text-[#006063]">
            ...
            <div className="pt-4 border-t border-gray-200 flex flex-col gap-2">
              <button
                ...
                className="w-full py-2.5 rounded-lg text-xs font-bold bg-slate-100 text-[#006063] border border-gray-300 flex items-center justify-center gap-2"
              >
                <Code2 className="w-4 h-4" />
              ...
              <button
                ...
                className="w-full py-2.5 rounded-lg text-xs font-bold bg-[#006063] text-white flex items-center justify-center gap-2"
              >
