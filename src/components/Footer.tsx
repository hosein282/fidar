      ...
      <div className="max-w-[1440px] mx-auto space-y-12">
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-baseline justify-between gap-8 pb-8 border-b border-gray-800">
          ...
          <div className="flex gap-4">
            <a href="https://www.facebook.com/BiesseHQ/" target="_blank" rel="noreferrer" className="w-11 h-11 rounded-full border border-white flex items-center justify-center text-white hover:bg-white hover:text-black transition">
              <Facebook className="w-5 h-5" />
            </a>
            <a href="https://www.instagram.com/biesse/" target="_blank" rel="noreferrer" className="w-11 h-11 rounded-full border border-white flex items-center justify-center text-white hover:bg-white hover:text-black transition">
              <Instagram className="w-5 h-5" />
            </a>
            <a href="https://www.linkedin.com/company/biesse" target="_blank" rel="noreferrer" className="w-11 h-11 rounded-full border border-white flex items-center justify-center text-white hover:bg-white hover:text-black transition">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-xs text-slate-400">
          ...
          <div>
            <h4 className="text-white font-bold text-sm mb-3">{isFa ? 'درباره گروه بیئس' : 'About Biesse'}</h4>
            <p className="text-xs leading-relaxed text-slate-400">
              {isFa ? (
                'بیئس یک شرکت بین‌المللی پیشرو در تولید خطوط و دستگاه‌های صنعتی، دیتابیس‌های قدرتمند و پلتفرم‌های دیجیتال است.'
              ) : (
                'Biesse is an international company producing lines, machines and components for transforming materials into products.'
              )}
            </p>
          </div>
        </div>
